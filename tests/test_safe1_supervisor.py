...
impossible to cause by a crashing classifier."""
import numpy as np
...
def test_single_overrun_does_not_demote(monkeypatch):
    v = SafeVad(prefer_silero=False)        # energy mode
...
def test_sustained_overrun_demotes(monkeypatch):
    ...
    for _ in range(SafeVad.OVERRUN_LIMIT):
        v.prob(f)
    assert v.mode == "open"                  # energy demoted after sustained stall
